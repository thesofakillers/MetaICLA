...
def main():
    ...
    if not os.path.exists(os.path.join(args.data_dir, "unifiedqa.zip")):
        download_from_google_drive(
            "1fybWU3eeN1jwFPuNa13JrLHzl5NGcYfB",
            os.path.join(args.data_dir, "unifiedqa.zip"),
        )
    ...
    if args.do_train:
        train_tasks = [
            t[len(prefix) :] for t in config["train"] if t.startswith(prefix)
        ]
        process_train(
            args.data_dir, train_tasks, args.max_length, k=args.train_k, seed=100
        )

    if args.do_test:
        test_tasks = [t[len(prefix) :] for t in config["test"] if t.startswith(prefix)]
        process_test(
            args.data_dir, test_tasks, k=args.test_k, seeds=[100, 13, 21, 42, 87]
        )
    ...
    for task in train_tasks:
        ...
        for line in tqdm(lines):
            ...
            if (
                ...
                and len(input_.split(" ")) > max_length
            ):
        ...
        with open(
            os.path.join(
                data_dir,
                prefix + task,
                "{}_{}_{}_train.jsonl".format(prefix + task, k, seed),
            ),
            "w",
        ) as f:
            for line in data:
    ...
    for task in test_tasks:
        ...
        with open(
            os.path.join(
                ...
                prefix + task,
                "dev.tsv"
                if task in ["mctest", "multirc", "qasc", "qasc_with_ir"]
                else "test.tsv",
            ),
            "r",
        ) as f:
            test_data = [
                _prepro(task, "test", line) for line in f if len(line.strip()) > 0
            ]
        ...
        for seed in seeds:
            ...
            with open(
                os.path.join(
                    data_dir,
                    prefix + task,
                    "{}_{}_{}_test.jsonl".format(prefix + task, k, seed),
                ),
                "w",
            ) as f:
