...
def load_prompts(do_train):
    ...
    for task, subtask in available_tasks.items():
        ...
        if do_train:
            ...
            all_template_names = [
                name for name in prompts.all_template_names if "no_option" not in name
            ]
            ...
            for keyword in ["multiple_choice", "most_correct", "most_suitable"]:
                _all_template_names = [
                    name for name in all_template_names if keyword in name
                ]
...
def apply_prompt(task, example, do_train, prompt_names_per_task, prompt_dict):
    ...
    if task == "commonsense_qa":
        ...
        assert options == ["no", "yes"] and output_ in ["yes.", "no."], (
            output_,
            options,
        )
        output_ = output_[:-1]
    ...
    return json.dumps(
        {"task": "inst:" + task, "input": input_, "output": output_, "options": options}
    )
...
def preprocess(dataset, line, config):
    ...
    do_handle_sep = dataset.startswith("race-") or dataset in [
        "sciq",
        "social_i_qa",
        "wiqa",
        "quail",
    ...
    if config["task_type"] == "multi-choice":
        ...
        if len(sentences) > 1:
            ...
            if dataset == "quarel":
                ...
                if (
                    output_ not in options
                    and output_.endswith(" or")
                    and output_[:-3] in options
                ):
                    output_ = output_[:-3]
    ...
    return json.dumps(
        {"task": dataset, "input": input_, "output": output_, "options": options}
    )
